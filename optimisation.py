...
class Optimisation:
	...
	def bayesian_optimisation(self, max_iterations=50):
		...
		sorted_laps = sorted(self.__initial_strategy.keys())

		# Construct the initial parameters dictionary
		initial_params = {
			"start_tyre": self.__initial_strategy[1],  # Starting tyre (lap 1)
			"num_pit_stops": len(self.__initial_strategy) - 1,  # Number of pit stops
			...
			"pit1_tyre": self.__initial_strategy[sorted_laps[1]] if len(sorted_laps) > 1 else 0,  # First pit stop tyre
			"pit2_tyre": self.__initial_strategy[sorted_laps[2]] if len(sorted_laps) > 2 else 0,  # Second pit stop tyre
			"pit3_tyre": self.__initial_strategy[sorted_laps[3]] if len(sorted_laps) > 3 else 0,  # Third pit stop tyre
		}
	...
	def genetic_algorithm_optimisation(self, population_size=50, generations=50):
		...
		sorted_laps = sorted([lap for lap in self.__initial_strategy.keys() if lap > 1])  # Sort pit stop laps
		...
		initial_params = {
			...
			"pit1_tyre": self.__initial_strategy[sorted_laps[0]] if len(sorted_laps) > 0 else 0,  # First pit stop tyre
			"pit2_tyre": self.__initial_strategy[sorted_laps[1]] if len(sorted_laps) > 1 else 0,  # Second pit stop tyre
			"pit3_tyre": self.__initial_strategy[sorted_laps[2]] if len(sorted_laps) > 2 else 0,  # Third pit stop tyre
		}
		...
		def create_initial_strategy():
			...
			return [
				random.choice(__unique_tyre_types) if random.random() < 0.2 else start_tyre,  # Starting tyre
				random.randint(max(2, pit1_lap - 5), min(pit1_lap + 5, self.race_data.max_laps - 1)) if pit1_lap > 0 else 0,  # Pit1 lap
				random.choice(__unique_tyre_types) if random.random() < 0.2 else pit1_tyre if pit1_tyre != 0 else random.choice(__unique_tyre_types),  # Pit1 tyre
				random.randint(max(2, pit2_lap - 5), min(pit2_lap + 5, self.race_data.max_laps - 1)) if pit2_lap > 0 else 0,  # Pit2 lap
				random.choice(__unique_tyre_types) if random.random() < 0.2 else pit2_tyre if pit2_tyre != 0 else random.choice(__unique_tyre_types),  # Pit2 tyre
				random.randint(max(2, pit3_lap - 5), min(pit3_lap + 5, self.race_data.max_laps - 1)) if pit3_lap > 0 else 0,  # Pit3 lap
				random.choice(__unique_tyre_types) if random.random() < 0.2 else pit3_tyre if pit3_tyre != 0 else random.choice(__unique_tyre_types),  # Pit3 tyre
			]
	...
	def simulated_annealing_optimisation(self, max_iterations=50):
		...
		__unique_tyre_types = sorted(self.race_data.get_unique_tyre_types())
		num_tyres = len(__unique_tyre_types)
		...
		def objective_function(params):
			...
			pit1_tyre = self.__map_to_nearest_tyre(params["pit1_tyre"])
			pit2_tyre = self.__map_to_nearest_tyre(params["pit2_tyre"])
			pit3_tyre = self.__map_to_nearest_tyre(params["pit3_tyre"])
		...
		start_tyre = __unique_tyre_types.index(self.__initial_strategy[1]) + 1  # Starting tyre index
		sorted_laps = sorted([lap for lap in self.__initial_strategy.keys() if lap > 1])
		...
		pit1_tyre = __unique_tyre_types.index(self.__initial_strategy[sorted_laps[0]]) + 1 if num_pit_stops >= 1 else 1
		pit2_tyre = __unique_tyre_types.index(self.__initial_strategy[sorted_laps[1]]) + 1 if num_pit_stops >= 2 else 1
		pit3_tyre = __unique_tyre_types.index(self.__initial_strategy[sorted_laps[2]]) + 1 if num_pit_stops >= 3 else 1
	...
	def monte_carlo_tree_search(self, max_iterations=100, exploration_weight=1.414):
		...
		def _generate_child_strategies(current_strategy):
			...
			if num_pits < 3:
				...
				if min_lap <= max_lap:
					...
					available_tyres = [tyre for tyre in self.__unique_tyre_types if tyre != current_strategy[last_pit]]
					if not available_tyres:
						available_tyres = self.__unique_tyre_types
			...
			for lap in list(current_strategy.keys()):
				...
				available_tyres = [tyre for tyre in self.__unique_tyre_types if tyre != current_tyre]
				if available_tyres:
					new_tyre = random.choice(available_tyres)
					new_strat = current_strategy.copy()
		...
		root = Node(self.__initial_strategy)
		...
		for _ in range(max_iterations):
			...
			# Simulation
			try:
				sim = RaceSimulator(self.race_data, self.overtake_model, self.__given_driver, node.strategy)
				sim_data = sim.simulate()
				position = next(d["position"] for d in sim_data if d["driver_number"] == self.__given_driver)
		...
		for strat in unique_strategies[:10]:
			try:
				sim = RaceSimulator(self.race_data, self.overtake_model, self.__given_driver, strat)
				sim_data = sim.simulate()
				pos = next(d["position"] for d in sim_data if d["driver_number"] == self.__given_driver)
			except:
				pos = 20
